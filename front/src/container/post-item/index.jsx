...
export default function Container({ id, username, text, date }) {
    ...
    const convertData = ({ post }) => ({
        ...
        reply: post.reply.reverse().map(({ id, username, text, date }) => ({
            id, username, text, date: getDate(date)
        })),
                ...
                <div style={{ padding: '0 20px 20px 20px' }}>
                    <Grid>
                        <Box className='post-item__inside-box'>
                            <PostCreate placeholder='Post your reply' button='Reply' id={data.id} onCreate={getData} />
                        </Box>
                        ...
                        {status === LOAD_STATUS.SUCCESS && !data.isEmpty && data.reply.map((item) => (
